...
class EEGNet_Final(nn.Module):
    def __init__(self, channels=22, samples=513, n_classes=4):
        ...
        self.dropout = nn.Dropout(0.5) # Slightly lowered for fine-tuning
...
if __name__ == "__main__":
    ...
    # Phase 1 Criterion (Strict)
    criterion_pre = nn.CrossEntropyLoss(label_smoothing=0.1)
    # Phase 2 Criterion (Confident/No Smoothing)
    ...
    # --- PHASE 2: FINE-TUNE (LIMIT BREAK) ---
    fine_epochs = 300
    swa_start = 220
    
    # Lower weight decay so the model can fit Subject 1
    ...
    # Tighter SWA Learning Rate
    swa_scheduler = SWALR(optimizer_fine, swa_lr=0.0001) 

    history = {'loss': [], 'val_acc': [], 'lr': []}
    best_acc = 0

    print(f"\n🔥 PHASE 2: LIMIT BREAK FINE-TUNING (SWA at {swa_start})...")
    ...
    ax1.axvline(x=swa_start, color='purple', linestyle='--', linewidth=2, label='SWA Start')
    
    lines_1, labels_1 = ax1.get_legend_handles_labels()
    lines_2, labels_2 = ax2.get_legend_handles_labels()
    ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper left')

    plt.title(f"BCI IV 2a Limit Break | Peak Single: {best_acc:.2f}% | Final SWA: {final_swa_acc:.2f}%")
    plt.tight_layout()
    plt.savefig("step35_limit_break_audit.png")
    print("\n✅ Audit Plot saved as 'step35_limit_break_audit.png'")
    plt.show()