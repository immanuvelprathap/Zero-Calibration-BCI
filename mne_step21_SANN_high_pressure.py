...
class SANN_PowerNet(nn.Module):
    def __init__(self, channels=64, num_subjects=100):
        ...
        # Shared Feature Extractor (Shallow ConvNet Style)
    ...
    def forward(self, x, alpha=1.0):
        ...
        # Path 1: Motor Imagery
        label_out = self.label_classifier(features)
        
        # Path 2: Subject Adversary
        rev_features = ReverseLayerF.apply(features, alpha)
        subject_out = self.subject_classifier(rev_features)
        
        return label_out, subject_out

# ==========================================
# 3. DATA PREP (Euclidean Alignment + ID Tagging)
# ==========================================
...
def load_adversarial_data(num_subjects=109):
    ...
    print(f"🚀 PREPPING {num_subjects} SUBJECTS FOR HIGH-PRESSURE SANN...")
    for sub_idx in range(num_subjects):
        try:
            ...
            if sub_id % 10 == 0: print(f"✅ Indexed {sub_id}")
        except Exception: continue
    return X_list, y_list, s_list

# ==========================================
# 4. TRAINING WITH WEIGHTED ADVERSARY
# ==========================================
if __name__ == "__main__":
    ...
    train_loader = DataLoader(TensorDataset(
        torch.from_numpy(X_train), torch.from_numpy(y_train), torch.from_numpy(s_train)), 
    ...
    print(f"\n🔥 STARTING HIGH-PRESSURE SANN | ALPHA SCALING: FAST")
    for epoch in range(100):
        model.train()
        # Fast Alpha Scaling: Reaches full pressure by Epoch 10
        alpha = min(1.0, (epoch + 1) / 10.0)
        
        t_l_loss, t_s_loss = 0, 0
        for b_x, b_y, b_s in train_loader:
            b_x, b_y, b_s = b_x.to(device), b_y.to(device), b_s.to(device)
            optimizer.zero_grad()
            l_out, s_out = model(b_x, alpha=alpha)
            
            loss_label = criterion(l_out, b_y)
            loss_subject = criterion(s_out, b_s)
            
            # 🔥 WEIGHTED PENALTY: 2.0x Subject Loss
            total_loss = loss_label + (2.0 * loss_subject)
            total_loss.backward()
            optimizer.step()
            
            t_l_loss += loss_label.item()
            t_s_loss += loss_subject.item()

        model.eval(); val_accs = []
        with torch.no_grad():
            for xt, yt in zip(X_val, y_val):
                p, _ = model(torch.from_numpy(xt).to(device), alpha=0)
                acc = (torch.max(p, 1)[1] == torch.from_numpy(yt).to(device)).sum().item() / len(yt)
                val_accs.append(acc)
        
        avg_v = np.mean(val_accs) * 100
        print(f"Epoch {epoch+1:02d} | Task: {t_l_loss/len(train_loader):.3f} | Subj: {t_s_loss/len(train_loader):.3f} | Val: {avg_v:.2f}% | α: {alpha:.2f}")

        if avg_v >= 95.0:
            print("🎯 BREAKTHROUGH."); break